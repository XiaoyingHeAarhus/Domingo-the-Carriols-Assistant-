// POST /api/reminder — save email reminder & attempt to send via EmailJS
...
exports.handler = async (event) => {
  ...
  // Basic email validation (no personal data stored beyond what user explicitly provides)
  ...
  try {
    ...
    const reminder = { id: "r-" + Date.now(), meetingId, email, minutesBefore: minutesBefore || 60, createdAt: new Date().toISOString() };
    reminders.push(reminder);
    await store.set("reminders", JSON.stringify(reminders));

    // Send immediate confirmation email via EmailJS REST API (if configured)
